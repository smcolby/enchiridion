...
- **Global activation on harnesses without native glob-scoped rules uses the degradation ladder's router form**: `enchiridion sync` validates rule schemas and generates the router skill's index from rule frontmatter, with rule bodies reachable through the skill directory. One skill wiring, progressive disclosure, no per-rule churn in the skill registry as the catalog grows.
- **Global activation on harnesses with a user-level rule directory rides the distribution layer directly**: `enchiridion sync` renders scoped rules into a committed per-harness directory, `bootstrap` symlinks it to the live location, and `verify` checks the render like any other generated file.
...
- **verify() extends the distribution pattern's congruence tool** with rule schema validation and the doctrine token ceiling (the ceiling declared in the registry, so generator and verifier read one number); `enchiridion doctor` provides the corresponding rules and live-wiring view. The audit schedule is a harness scheduling feature or a calendar reminder; it needs no repo infrastructure.
