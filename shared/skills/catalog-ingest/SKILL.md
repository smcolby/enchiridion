...
Run `uv run enchiridion sync --rules --apply` and `uv run enchiridion verify` from the repo root; commit the canonical files and any regenerated router index together, citing the source.
